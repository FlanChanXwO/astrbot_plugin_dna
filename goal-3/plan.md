...
在独立工作树中实现“客户端更新”和“订阅客户端更新”能力：基于更新 API 契约查询国服 PC/安卓最新版本，保存定时观察基线，比较版本变化和本次新增更新大小，并向群聊订阅目标定时推送。当前轮次只完成 goal-mode 初始化，不实施业务代码。
...
Task 01 已完成规格固化与独立审查；后续每轮只执行 `tasks.md` 中第一个未完成 task。Task 02 起进入 TDD：先写并运行 Red 测试，再实现对应业务代码。当前已确认规格仍不授权本轮提前实现业务代码。
