...
| `dna_code` | `兑换码`、`cdk`、`code` | 公共资源仓库 Raw provider | 按 v1 契约过滤并展示当前有效码及已提供字段的 `ChainResponse` |
...
`alias_list` 的 owner 权限是 legacy 契约的一部分；别名添加、删除和恢复写命令已从
rewrite registry、帮助和 bootstrap 服务中移除，当前只保留读取能力。
...
- API transport 在边界把 legacy Pydantic model 映射为 typed snapshot；网络、状态码、
  服务端和兑换码契约错误不把响应原文或 secret 带进用户响应。
...
3. legacy 兑换码 URL 固定在 handler 内；rewrite 将 provider 注入 transport，默认实现
   使用公共资源仓库 Raw URL，并沿用 GitHub 加速前缀，fixture 不联网。网络、HTTP 状态码
   或契约错误以可见的稳定失败文案表达。
4. legacy 某些素材缺失时会返回空列表或抛出底层图片错误；rewrite 对缺失资源给出明确
   未找到/placeholder 语义，不把空结果当作成功资料。
5. legacy 兑换码只展示第一个有效码的截止时间；rewrite 按资源仓库 v1 的
   `valid_from`/`expires_at` 过滤，并将每个当前有效码与奖励、时间、平台和区服等已提供
   字段放在同一条消息链项中。人工审查结论为接受：否则会丢失合法 provider 数据，且该
   差异已有逐码 fixture 回归测试。
