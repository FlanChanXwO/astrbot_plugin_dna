...
### P1：资源文档、同步目录和生产 renderer 尚未形成闭环
...
Task 16.1 负责建立可验证的运行期资源契约和 bootstrap 接线。它不创建或访问外部私有仓库，
只用隔离素材 fixture 验证布局和资源 metadata。

### P2：每次图片查询都保留新的运行期 PNG

玩家与百科 renderer 每次以 UUID 生成 `rendered/*.png`，入口没有把合成图注册到 AstrBot
事件的公开临时文件清理机制。长期运行会持续保留含角色/UID信息的图片。Task 16.1 应使用
发送完成后的事件生命周期清理合成图；原图资产的保留规则需独立处理，不能以固定时间或条数
静默删除。
...
- 兑换码 provider、日历和资源均保持 `fixture/待实测` 或差异待审查状态；没有在本轮联网、
  创建资源仓库或执行真实 NapCat。
- 命令矩阵和用户文档已改为披露原图与资源的当前限制。修复任务完成前，不得将这些能力的
  实现存在、资源同步成功或图片生成成功解释为真实行为通过。

## 后续

按顺序执行 Task 16.1（资源和合成图片生命周期）与 Task 16.2（原图映射和伤害输出）。完成后
重跑完整门禁，并在 Task 30 以只读矩阵补充人工接受结论。
