...
- `cache/` — 玩家数据 JSON、完整 T2I 图片卡片以及公告、密函等 `CacheManager` 内容；所有
  条目共用 `cache.ttl_hours`，默认 24 小时，身份相关 key/tag 不保存原始 user_id 或 UID。
  `-1` 表示永久有效且不因时间清理，`0` 表示不读写持久内容缓存，正数表示到期后直接 miss；
