...
| `cache.refresh_send_card` | `true` | 单角色面板或基本信息卡片主动刷新成功后是否在成功提示后发送新图片。`false` 仍完整刷新、渲染并更新对应缓存，只省略图片；批量刷新始终只返回汇总。 |
...
单角色主动刷新成功时固定先返回 `角色【正式名】面板已刷新`；基本信息卡片刷新固定先返回
`基本信息卡片缓存已刷新`。`refresh_send_card=true` 时随后附带新卡片图片，`false` 时仅返回文字，
但不会跳过刷新、渲染或缓存更新。刷新全部角色命令继续只返回成功/失败汇总，不逐张发送图片。

## 登录边界与 AstrBot 权限

`permission=user/admin` 是 AstrBot 命令权限，不等同于 DNAUID 账号登录。密函组、日历、公告查看、
图鉴、别名、攻略、兑换码、客户端更新查询/订阅/取消订阅、个人隐私设置以及资源状态/下载等
公开资料或管理命令不要求调用者绑定账号；客户端更新订阅/取消订阅仍需要 AstrBot `admin` 权限并在
群聊中执行。密函查询使用系统任意可用账号凭据，没有可用凭据时会提示管理员完成登录。

角色卡片、角色详情、角色面板、体力、周报和签到等当前 UID 数据命令仍要求绑定当前 UID，并继续
执行隐私控制；个人隐私设置本身无需登录。基本信息卡片刷新/清理命令需要当前 UID，但不会刷新角色
面板；`cache.refresh_send_card` 同样适用于基本信息卡片刷新。`group_report` 等签到配置与本轮登录
边界和缓存命令改动无关。
