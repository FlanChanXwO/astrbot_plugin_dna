...
契约。Target 表示“区服 × 账号生态 × 平台”，而 Steam、TapTap、WeGame 等单纯
下载入口并不自动形成新的账号生态或 Target。可选 Target 以 `_conf_schema.json`
中的生成 options 为准。
...
`user` / `admin` 表示 AstrBot 命令权限；DNA 账号是否已经绑定/登录是另一个业务条件。公开资料类命令可能不需要游戏账号，而玩家数据、签到等命令会要求有效绑定。管理员命令也不会因为调用者已经登录就自动获得 AstrBot 管理权限。
