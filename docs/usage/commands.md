...
当前 main 注册 `commands.json` 中的 61 条命令，覆盖账号/登录、玩家查询、资料读取、隐私、签到、通知、别名和资源管理；生产版不再公开面板管理和密函测试命令。命令声明位于
...
- `绑定<13位UID>`、`切换<13位UID>`、`删除<13位UID>`：管理当前账号的 UID 绑定；删除后可继续查看其余绑定。
