# 命令

插件默认命令前缀为 `kk`，例如 `kk帮助`、`kk签到`、`kk登录`；实际触发前缀以 Dashboard
配置项 `display.command_prefixes` 为准。为保持文档紧凑，下文命令列表省略前缀；发送时请
在命令主体前加上当前配置的前缀。

当前 main 注册 `commands.json` 中的 63 条命令，覆盖账号/登录、玩家查询、资料读取、隐私、签到、通知、别名、资源管理和客户端更新；生产版不再公开面板管理和密函测试命令。命令声明位于
`src/modules/index.py` 引用的模块中，`commands.json` 是由
`scripts/generate_commands_manifest.py` 生成的可审阅清单；以当前配置前缀发送 `帮助` 查看同一
registry 的帮助文本。

更新或回滚插件时，生产环境按发布清单使用精确插件 SHA、已认证的 Dashboard 定向 reload 和同一
接口回滚；不要把容器重启或未经认证的 HTTP `2xx` 当作发布成功。具体的部署、备份和回滚边界见
[维护说明](../dev/maintenance.md)与[发布清单](../porting/agent-tools-release-checklist.md)。

## Agent Tools

Agent Tools 不属于聊天命令和 `commands.json`，不使用命令前缀，也不会出现在聊天帮助中。它们
由独立的 `agent_tools.enabled` 开关控制，默认关闭；工具名称、参数、结构化返回、身份边界和
签到安全说明见 [Agent Tools 使用说明](agent-tools.md)。

## 当前账号命令

- `登录`、`登陆`、`登入`、`登龙`、`login`：启动登录页会话并立即返回链接；支持 local、http_poll、sse、ws
  四种 typed transport，重复发起会复用同一消息作用域的等待会话。
- `登录` 加 40 个字符以上 token：执行 token 登录；例如 `登录<token>`。
- `token登录<token>`、`登录token<token>`：账号管理中的显式 token 登录入口。
- `登录手机号,验证码`：执行手机号验证码登录，例如 `登录13800138000,1234`。
- `退出登录`、`登出`、`logout`：退出当前 active UID 的登录。
- `切换<13位UID>`、`删除<13位UID>`：管理当前账号已有的 UID；删除后可继续查看其余绑定。
- `删除全部UID`、`查看UID`：删除全部或查看当前作用域的绑定。
- `获取ck`、`获取Token` 等别名：只查询 App 凭据保存状态，严格不返回原始凭据；Web 凭据和 Web fallback 已移除。

默认 local runtime 会在插件初始化时启动内置登录页，`登录` 会先返回实际监听地址；插件
终止时取消等待任务并释放端口。外置 transport 未配置地址或回执不是 App 凭据时返回稳定
失败提示，不发送伪造链接。测试使用 fake transport，不执行真实短信、token 或外部登录服务。

## 玩家查询命令

- `查询`、`卡片`、`角色`、`信息`：读取当前 UID 的角色、近战武器和远程武器总览。
- `<角色名>面板`、`<角色名>信息`、`<角色名>详情`、`<角色名>面包`、`<角色名>🍞`：读取
  角色属性、技能、溯源、魔之楔和武器等基础详情；可追加一把近战和一把远程武器，
  例如 `角色名面板 +近战名 +远程名`。正常路径不请求伤害计算 API，也不渲染伤害区块。
- `刷新<角色名>面板`：普通用户强制刷新自己当前 UID 下的指定角色；刷新数据后是否立即发送新卡片由
  `cache.refresh_send_card` 控制。
- `刷新全部角色面板`：刷新自己当前 UID 的概览和全部已解锁角色面板数据，只返回成功/失败汇总，
  不逐张发送图片。
- `清理<角色名>面板缓存`：清理自己当前 UID 的指定角色数据和卡片缓存，保留概览缓存。
- `清理全部角色缓存`：清理自己当前 UID 的全部玩家数据和卡片缓存，不影响公告等其它缓存类型。
- `刷新<游戏UID>的<角色名>面板`：管理员按指定游戏 UID 刷新角色面板；凭据仍使用管理员当前账号作用域。

玩家图片由 `ImageResponse` 交给 AstrBot 公共 `image_result`，实际 T2I 图片写入
`StarTools.get_data_dir("astrbot_plugin_dnaby")/rendered/`。概览/详情渲染保留所有
合法角色、武器、技能和魔之楔等基础字段；正常路径不调用伤害计算 API，也不渲染伤害区块。
伤害数据结构、renderer、模板和 CSS 仍保留给后续显式计算调用复用。图片 artifact 元数据只用于
离线布局/资源语义回归；生成的 JPEG/PNG 与 sidecar/manifest 会在本次事件中登记到 AstrBot
的临时文件生命周期，并由后台维护任务按
合成 `rendered/*.png` 会在本次事件中登记到 AstrBot 的临时文件生命周期，并由后台维护任务按
保留期清理已脱离事件的孤儿文件；已登记的发送中图片通过租约保护。运行期资源中的原图、wiki
和攻略图片不会被登记为临时文件。
真实账户只读矩阵留待后续阶段，见 [v0.3 玩家行为矩阵](../porting/review-v0.3-player.md)。

## 资料读取命令

- `每日`、`mr`、`便笺`、`体力`、`日常` 等：读取实时便笺和锻造状态。
- `周报`、`本周周报`、`上周周报`：读取对应周期的资源获取统计。
- `日历`：读取活动日历；它不读取或修改账号数据。
- `<名称>图鉴`、`<名称>wiki`：按角色、武器或魔灵的运行期资源索引返回图鉴图片。
- `<角色名>攻略`：按已配置的攻略作者返回单图，或每位作者一条文案后跟随其全部图片的消息链。
- `兑换码`、`cdk`、`code`：读取资源仓库中当前有效的兑换码；奖励、生效时间、截止时间、平台和区服等已提供字段会随各码显示。
- `<角色/武器名>别名`：查看只读别名列表，使用 `user` 权限。
- `角色列表`、`武器列表`：查看运行期资源中已索引的 canonical 名称，使用 `user` 权限。

## 客户端更新命令

- `客户端更新`：查询国服 PC 和安卓最新客户端版本；`客户端更新 PC` 或 `客户端更新 安卓`
  只查询指定平台。查询只读，不推进定时任务维护的历史基线。
- `订阅客户端更新`：由群聊管理员为当前群订阅全部平台；追加 `PC` 或 `安卓` 可只订阅一个
  平台。`取消订阅客户端更新`：取消当前群的客户端更新订阅。
- 订阅按当前会话目标保存，平台选择写入 `Subscription.extra_data` 的规范化 `platforms` 字段；
  重复订阅同一类型和目标会更新平台筛选而不是创建重复记录。首次订阅或首次成功检查只建立基线，
  不补发无法确认时间范围的历史更新；首次检查失败仍保留订阅，下一次成功检查继续建立基线。
- `notifications.client_update_enabled` 启用后，定时任务按 `notifications.client_update_check_minutes` 独立检查；
  确认版本变化时，每个平台默认发送一条消息，展示区服、平台、旧版本、新版本和新增更新大小。PC/安卓相互独立，单个平台
  失败只记录安全日志，不向用户伪称“无更新”。
- OneBot 且 `notifications.client_update_merge_forward` 开启时，同轮多平台消息会尝试合并转发；
  非 OneBot 不受此开关影响。合并能力不可用或失败时安全降级为逐平台普通消息。

> 当前迁移阶段已完成查询、群聊订阅、独立轮询、基线状态、定时推送接线和框架无关投递边界。客户端更新
> 变化会以 `event_key` 固定首次匹配目标，逐目标记录 `pending`/`delivered`，下一轮先重试失败目标；取消或
> 停用会清理待投递目标，重新订阅不会补发旧事件。

## 签到命令

- `签到`、`社区签到`、`每日任务`、`社区任务`、`库街区签到`、`sign`：对当前 active
  UID 固定执行游戏签到和配置列表中的社区任务（签到/浏览/点赞/分享/回复），返回逐项状态；
  今日已完成直接提示重复签到。真实写操作只通过注入的 `CheckinTransport` 执行，
  测试使用 fake transport。
- `签到日历`、`签到记录`、`签到历史`：渲染签到日历（皎皎积分、社区/游戏累计签到、
  社区任务进度和逐日奖励），使用 `user` 权限。
- `全部签到`：对所有已登录账号执行签到并按并发/间隔聚合成功/失败结果，使用 `admin`
  权限。真实账户不执行批量写操作，仅离线 fixture 覆盖契约。
- `开启自动签到`、`关闭自动签到`：按当前 `(user_id, UID)` 绑定保存自动签到开关；切换 UID
  后各 UID 独立。每日计划任务只签到开启的绑定，`sign_in.enable_all_users` 开启时强制全部签到，
  手动 `全部签到` 不受此开关影响。
- `订阅签到结果`、`取消订阅签到结果`：在 `sign_in.scheduled_enabled` 开启时，每日
  `sign_in.sign_time` 由计划任务执行全账号自动签到，并把摘要推送给订阅者；订阅按
  type+会话去重，使用 `admin` 权限。真实推送只走注入的 `context.send_message`，
  离线测试用 fixture 验证。

## 通知命令

- `密函`、`委托密函`、`mh`：读取当前小时段的密函委托（角色/武器/魔之楔分节），默认渲染
  1700×900 运行期 T2I 图片（常规输出 JPEG）；开启简洁图片模式时按分栏数量生成对应宽度；需要调用者 active UID 的凭据。
- `密函列表`、`mh_list`：返回 legacy 密函委托名称清单，无需账号。
- `公告`：读取并渲染完整公告列表，transport 会按服务端分页继续读取；`公告 序号`（如
  `公告 1`）读取对应公告详情。详情正文的多页图片会在同一条回复中连续发送，图片源地址保留
  query/hash 和无扩展名 CDN 形式；列表/详情无需账号凭据。详情图片或列表预览失败时，手动
  查询返回固定失败文案，不发送占位图。
- 活动日历（`日历`）已在资料读取命令中提供，本阶段不重复注册。

### 通知订阅命令

- `订阅<密函名>密函`、`取消订阅<密函名>密函`（如 `订阅拆解密函`）：按名称订阅/取消
  密函委托，支持 `角色|武器|魔之楔` 类型前缀与 `全部`（仅取消）；`user` 权限，按
  user+会话作用域去重。
- `我的密函`、`密函订阅`：查看当前密函订阅与推送时间窗口。
- `订阅密函时间17:23`、`订阅密函周期17:23`：设置密函推送时间窗口（`user`）。
- `订阅密函图片`/`取消订阅密函图片`、`订阅密函文本`/`取消订阅密函文本`：会话作用域
  图片/文本推送开关（`user`）。

### 公告管理（admin）

- `订阅公告`、`取消订阅公告`：群聊作用域公告推送开关。

- 密函自动推送默认在每小时整点（由 `notifications.secret_push_minute` 配置分钟），不提供全局
  密函缓存开关；当前小时数据未准备好或结构校验失败时按 `secret_retry_interval_seconds` 重试，
  成功后每小时只推送一次。订阅级时间窗口仍按
  `订阅密函时间17:23`/`订阅密函周期17:23` 过滤目标。公告按
  `notifications.announcement_check_minutes` 轮询并推送新条目；真实推送只走注入的
  `context.send_message`，离线测试用 fixture 验证。

## 资源管理命令（admin）

- `资源状态`：查看公共资源仓库、manifest 和当前 generation 状态；不读取或统计遗留 `panel_custom/`。
- `下载全部资源`：获取 `main` 的 `FETCH_HEAD`，先校验候选 generation，再以 fast-forward
  发布公共资源；Git、候选校验错误和本地修改均返回可见错误，上一份已验证资源继续可用。

## 别名维护（admin）

- `添加角色<角色名>别名<别名>`、`删除角色<角色名>别名<别名>`：维护角色 custom 别名。
- `添加武器<武器名>别名<别名>`、`删除武器<武器名>别名<别名>`：维护独立的武器 custom 别名文件。
- `恢复别名`：重新加载默认资源并保留 custom 别名；`强制恢复别名`：清空角色和武器 custom 别名。

角色、武器别名查看和 `角色列表`/`武器列表` 属于普通用户的“图鉴”功能；上述写操作使用 AstrBot
`ADMIN` 权限。

更新历史不再提供聊天命令，统一记录在仓库根目录的 `CHANGELOG.md`；因此不会出现在帮助或
`commands.json` 中。

资料读取的图片与索引只使用
`StarTools.get_data_dir("astrbot_plugin_dnaby")/resource_generations/`（首次生成前兼容读取
`resources/` 缓存）和 `rendered/`，不从插件源码目录写入或下载素材。图像语义、已知差异和
fixture 边界见
[v0.3 资料查询行为矩阵](../porting/review-v0.3-encyclopedia.md)。

资源同步后的完整目录已接入玩家与百科 renderer；metadata 会把实际读取到的素材标为
`provided`，缺失素材标为 `placeholder` 或 `fallback`。本地视觉 fixture 已由 O23 实际打开检查，
`atri` 的 O24 核验只证明部署状态与代码/资源可读性；真实 CDN/T2I 图片成功仍需按外部服务可用性单独记录。

玩家、图鉴、签到、密函和公告等查询命令支持在命令前后 `@` 指定目标；仅 @ 机器人自身时按
当前调用者查询。若平台没有交付可解析的 @ 目标，命令会提示“未识别到有效的 @ 目标”，不会
静默改查调用者。

## 隐私控制命令

个人命令（`user` 权限）：

- `开偷窥`、`关闭偷窥防护`：允许他人查看自己的游戏信息。
- `防偷窥`、`开启偷窥防护`：禁止他人查看自己的游戏信息。
- `隐藏UID`、`隐藏uid`：隐藏自己的 UID。
- `显示UID`、`显示uid`：显示自己的 UID。

隐私管理命令（`admin` 权限，必须在群聊使用）：

- `指定开偷窥`、`指定防偷窥`：对消息中被 `@` 且已绑定 UID 的目标设置偷窥权限。
- `全体开偷窥`、`全体防偷窥`、`取消全体偷窥`：设置、关闭或取消群组偷窥强制策略。
- `指定隐藏UID`、`指定显示UID`：对被 `@` 且已绑定 UID 的目标设置 UID 显示策略。
- `全体隐藏UID`、`全体显示UID`、`取消全体UID隐藏`：设置、关闭或取消群组 UID 强制策略。

群强制策略按字段优先于个人设置；个人命令在相应强制策略存在时会返回原因且不写入。
未迁移的 legacy 命令仍保留在重构区作为参考，但不会被新入口注册或展示。历史完整清单
和原行为见 `legacy-reference` 分支及 `docs/porting/`。

## 清单字段

每条命令由 `CommandSpec` 提供：`id`、`pattern`、`group`、`name`、`description`、
`examples`、`permission`、`use_case`。registry 会校验权限、正则、示例、重复 id/
pattern 和重复模块加载。

## 权限

- `user`：AstrBot `PermissionType.MEMBER`。
- `admin`：AstrBot `PermissionType.ADMIN`；当前所有原管理命令均归入此权限，具体以
  `commands.json` 和运行时 registry 为准。
