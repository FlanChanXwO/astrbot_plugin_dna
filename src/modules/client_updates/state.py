...
class ClientUpdatePendingTarget:
    ...
    def __post_init__(self) -> None:
        ...
        normalized_target_ids = _normalize_target_ids(self.target_ids)
...
class ClientUpdateStateStore:
    ...
    async def get_baseline(self, source_id: str) -> ClientUpdateBaseline | None:
        ...
        normalized_source_id = _registered_source_id(source_id)
        await self.load()
    ...
    async def save_baseline(self, baseline: ClientUpdateBaseline) -> None:
        ...
        normalized = _canonicalize_baseline(baseline)
        async with self._lock:
            self._load_unlocked()
    ...
    async def save_baseline_with_pending_event(
        ...
        normalized_baseline = _canonicalize_baseline(baseline)
        normalized_change = canonicalize_client_update_change(change)
    ...
    def _load_unlocked(self) -> None:
        ...
        try:
            ...
            for raw_source_id, raw_baseline in raw_baselines.items():
                source_id = _registered_source_id(raw_source_id)
                baseline = _parse_baseline(
                    raw_baseline, f"state.baselines[{raw_source_id!r}]"
                )
...
def canonicalize_client_update_change(
    ...
    if not isinstance(change, ClientUpdateChange):
        raise TypeError("change 必须是 ClientUpdateChange")
    source_id = _registered_source_id(change.source_id)
...
def _canonicalize_baseline(baseline: ClientUpdateBaseline) -> ClientUpdateBaseline:
    if not isinstance(baseline, ClientUpdateBaseline):
        raise TypeError("baseline 必须是 ClientUpdateBaseline")
    source_id = _registered_source_id(baseline.source_id)
    if baseline.last_change is not None:
        change = canonicalize_client_update_change(baseline.last_change)
        if change.source_id != source_id:
            raise ValueError("baseline.last_change 与基线 Source 不一致")
    return baseline


def _registered_source_id(value: object) -> str:
    if not isinstance(value, str):
        raise TypeError("source_id 必须是字符串")
    return resolve_client_update_source(value).source_id
...
def _normalize_target_ids(value: object) -> tuple[str, ...]:
    ...
    for target_id in target_ids:
        resolve_client_update_target(target_id)
...
def _parse_baseline(value: object, context: str) -> ClientUpdateBaseline:
    ...
    return ClientUpdateBaseline(
        version=_parse_version(
            _required(entry, "version", context), f"{context}.version"
        ),
...
def _parse_version(value: object, context: str) -> ClientSourceVersion:
    entry = _require_mapping(value, context)
    source_id = _registered_source_id(_required(entry, "source_id", context))
    order_key = _parse_order_key(_required(entry, "order_key", context), context)
    metadata = _parse_provider_metadata(
        _required(entry, "provider_metadata", context), source_id, context
    )
...
def _parse_change(value: object, context: str) -> ClientUpdateChange | None:
    ...
    change = ClientUpdateChange(
        ...
        target_ids=_normalize_target_ids(_required(entry, "target_ids", context)),
    )
    return canonicalize_client_update_change(change)
...
def _parse_pending_events(value: object) -> dict[str, ClientUpdatePendingEvent]:
    ...
    for index, raw_event in enumerate(value):
        ...
        change = _parse_change(_required(entry, "change", context), f"{context}.change")
        if change is None:
        ...
        event = ClientUpdatePendingEvent(
            change=change,
            targets=tuple(
                _parse_pending_target(item, f"{context}.targets[{target_index}]")
