...
def decode_target_id(value: object) -> tuple[str, str, str] | None:
    ...
    if encode_target_id(Subscription(type=key[0], unified_msg_origin=key[1], uid=key[2])) != value:
        return None
    return key
...
class AnnouncementTargetService:
    ...
    async def subscribe(
        ...
        *,
        ...
        if not origin or not group_id or not bot_id:
            return TargetMutationResult(TargetMutationStatus.INVALID, message="公告目标身份不完整")
    ...
    async def disable(self, target_id: str) -> TargetMutationResult:
        ...
        if key is None:
            return TargetMutationResult(TargetMutationStatus.INVALID, message="target_id 无效")
        async with self._lock_for(key):
            target = await self._find(key)
            if target is None:
                return TargetMutationResult(TargetMutationStatus.NOT_FOUND, message="公告目标不存在")
            ...
            return TargetMutationResult(TargetMutationStatus.APPLIED, subscription=disabled)
    ...
    async def enable(self, target_id: str) -> TargetMutationResult:
        ...
        async with self._lock_for(key):
            ...
            try:
                ...
                await self.delivery_state.baseline_target(target.unified_msg_origin, ids)
            except Exception as error:  # noqa: BLE001
                return TargetMutationResult(
                    TargetMutationStatus.PARTIAL,
                    subscription=target,
    ...
    async def delete(self, target_id: str) -> TargetMutationResult:
        ...
        async with self._lock_for(key):
            ...
            return TargetMutationResult(TargetMutationStatus.APPLIED, subscription=target)
