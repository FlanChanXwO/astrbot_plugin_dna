...
async def send_board_cast_msg(msgs: dict, board_cast_type: BoardcastType):
    ...
    """
    logger.info("订阅推送任务启动 boardcast_type=%s", board_cast_type)
    ...
    for qid, entries in private_msg_list.items():
        try:
            ...
            logger.exception("订阅私聊推送失败 target=%s error=%s", qid, e)
        await asyncio.sleep(0.5 + random.randint(1, 3))
    logger.info("订阅私聊推送完成 boardcast_type=%s", board_cast_type)
    ...
    for gid, entries in group_msg_list.items():
        try:
            ...
            logger.exception("订阅群聊推送失败 target=%s error=%s", gid, e)
        await asyncio.sleep(0.5 + random.randint(1, 3))
    logger.info("订阅群聊推送完成 boardcast_type=%s", board_cast_type)
    logger.info("订阅推送任务结束 boardcast_type=%s", board_cast_type)
