...
def has_valid_at(ev: EventContext) -> bool:
    """检查是否存在有效的 @ 目标
    ...
    if not ev.at:
        return False
    # 过滤无效的 at 数据（如 QQ 官方平台传入平台名称而非用户 ID）
    return ev.at != ev.bot_id and ev.at != ev.real_bot_id


async def get_using_id(ev: EventContext) -> str:
    """获取要查询的用户ID
    ...
    from ..utils.database.models import DNAGroupPrivacy, DNAPrivacy
...
def is_peek_blocked(ev: EventContext, user_id: str) -> bool:
    """检查是否因偷窥权限被阻止查询他人数据
