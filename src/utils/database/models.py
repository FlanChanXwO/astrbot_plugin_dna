...
# 新装无需 ALTER 迁移（表随模型创建）；旧 gsucore 库承接不在本移植范围。
# SQLModel 0.0.39 的类型存根未暴露 table=True 元类参数，以下声明只精确忽略该存根缺口。
...
class DNABind(Bind, table=True):  # pyright: ignore[reportGeneralTypeIssues, reportCallIssue]
...
class DNAUser(User, table=True):  # pyright: ignore[reportGeneralTypeIssues, reportCallIssue]
    __table_args__: ClassVar[dict[str, Any]] = {"extend_existing": True}
    ...
    async def select_data_by_cookie_and_uid(
        cls, session: AsyncSession, cookie: str, uid: str
    ) -> Self | None:
    ...
    @classmethod
    @with_session
    async def get_dna_all_user(cls, session: AsyncSession) -> list[Self]:
        """获取所有有效用户"""
        sql = select(cls).where(
            and_(
                or_(col(cls.status) == null(), col(cls.status) == ""),
...
class DNASign(BaseIDModel, table=True):  # pyright: ignore[reportGeneralTypeIssues, reportCallIssue]
    __table_args__: ClassVar[dict[str, Any]] = {"extend_existing": True}
...
class DNAPrivacy(BaseIDModel, table=True):  # pyright: ignore[reportGeneralTypeIssues, reportCallIssue]
    """隐私设置表：存储用户的窥屏权限设置"""
...
class DNAGroupPrivacy(BaseIDModel, table=True):  # pyright: ignore[reportGeneralTypeIssues, reportCallIssue]
    ...
    async def set_group_force_privacy(
        ...
        group_id: str,
        bot_id: str,
        force_allow_peek: bool | None | Any = NO_CHANGE,
        force_uid_hidden: bool | None | Any = NO_CHANGE,
    ) -> Self:
        """设置群的强制隐私设置
