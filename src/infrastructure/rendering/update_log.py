...
TEXT_PATH = Path(__file__).parents[2] / "resources" / "textures" / "update"
BACKGROUND_PATH = Path(__file__).parents[2] / "resources" / "textures" / "common" / "bg.jpg"
FONT_ORIGIN_PATH = Path(__file__).parents[2] / "resources" / "fonts" / "dna_fonts.ttf"
EMOJI_ORIGIN_PATH = Path(__file__).parents[2] / "resources" / "fonts" / "NotoColorEmoji.ttf"
...
async def draw_update_log_img(logs: list[str] | None = None) -> bytes | str:
    ...
    title_img = TEXT_PATH / "log_title.png"
    if not title_img.exists():
        title_img = Path(__file__).parents[2] / "resources" / "textures" / "update" / "log_title.png"
    return await _RENDERER.render(
        "cards/update_log.html.j2",
        {
            ...
            "title_image": image_data_uri(title_img),
            "width": CARD_W,
        },
        RenderSpec(width=CARD_W, full_page=True, output_format="jpeg"),
    )
