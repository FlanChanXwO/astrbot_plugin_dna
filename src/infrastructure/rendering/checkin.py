...
async def _draw_sign_calendar_view(
    ...
    achievement_info = [
        ...
        {"label": "游戏累计签到", "value": str(calendar.signin_time or 0)},
    ]
    achievement_info.extend(
        {"label": item.param_key, "value": item.param_value}
        for item in role.params
        if item.param_key in ("总活跃天数",)
    )
    tasks = [
        {
            "done": task.complete_times >= task.times,
            "progress": max(0.0, min(task.process, 1.0)),
            "remark": task.remark,
        }
        for task in (() if task_process is None else task_process.daily_tasks)
    ]
    award_dict = {award.day_in_period: award for award in calendar.day_awards}
        ...
        if award:
            icon = pil_image_data_uri(
                await download_pic_from_url(
                    SIGN_PATH, award.icon_url, size=(140, 140)
                )
            )
        return {
            "amount": award.award_num if award else 0,
            "current": day == (calendar.signin_time or 0),
            "day": day,
            "icon": icon,
            "signed": day <= (calendar.signin_time or 0),
        }

    period = calendar.period
    over_days = period.over_days if period else 30
    awards = list(
...
async def _draw_sign_calendar(
    ...
    if isinstance(sign_data, SignCalendar) and isinstance(role_show, (RoleHeader, RoleOverview)):
        if isinstance(role_show, RoleOverview):
            role_show = RoleHeader(
                role_id=role_show.role_id,
                role_name=role_show.role_name,
                level=role_show.level,
                params=list(role_show.params),
            )
        return await _draw_sign_calendar_view(
            ctx,
            role_show,
            sign_data,
    ...
    async def render_calendar(
        ...
        *,
        ...
        sections = (
            {
                "name": "社区任务",
                "items": 0 if data.tasks is None else len(data.tasks.daily_tasks),
            },
            {"name": "游戏签到", "items": len(data.calendar.day_awards)},
        )
