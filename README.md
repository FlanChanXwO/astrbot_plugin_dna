...
默认命令前缀为 `kk`。如果修改了 `general.command_prefixes`，请同步替换下列示例中的前缀。

| 场景 | 示例 | 说明 |
| --- | --- | --- |
| 帮助 | `kk帮助` | 查看当前可用命令 |
| 登录 | `kk登录` | 发起账号登录流程 |
| UID 管理 | `kk查看UID` | 查看当前账号绑定情况 |
| 信息卡片 | `kk卡片` | 查询当前 UID 的基本信息卡片 |
| 角色面板 | `kk菲娜面板` | 查询指定角色详情 |
| 日常信息 | `kk日常` / `kk周报` / `kk日历` | 查看便笺、周报与日历 |
| 图鉴攻略 | `kk菲娜图鉴` / `kk菲娜攻略` | 查看角色图鉴或攻略 |
| 签到 | `kk签到` | 执行当前 UID 签到 |
| 资源状态 | `kk资源状态` | 检查公共资源版本与状态 |
| 同步资源 | `kk同步资源` | 管理员同步公共资源 |
...
- `kk资源状态`：查看当前 generation、资源版本与最近同步结果；
- `kk同步资源`：从 `dna-resource` 的 `main` 分支同步并发布经过校验的新资源快照。
