...
- **角色与玩家信息**：角色总览、角色详情、面板刷新、角色/基本信息卡片缓存清理、日常便笺、周报和日历。
...
| `cache.refresh_send_card` | `true` | 单角色面板或基本信息卡片主动刷新成功后是否发送新图片；关闭时仍完整刷新、渲染并更新对应缓存。 |

## 命令

默认前缀为 `kk`。下列示例均使用默认前缀；如果修改了 `general.command_prefixes`，请把示例中的 `kk` 替换为自己的前缀。完整的 66 条命令、参数和权限见 [`commands.json`](commands.json)，命令使用说明见 [`docs/usage/commands.md`](docs/usage/commands.md)。
...
| 基本信息卡片缓存 | `kk刷新卡片缓存`、`kk清理卡片缓存`、`kk删除卡片缓存` | 刷新或清理基本信息卡片，不操作角色面板；需要当前 UID。 |
...
账号绑定和命令权限彼此独立：`user`/`admin` 是 AstrBot 权限，不等同于是否已登录 DNAUID。
密函、日历、公告查看、图鉴、别名、攻略、兑换码、客户端更新三条命令，以及个人隐私设置、
资源状态/下载等公开资料或管理命令不要求调用者绑定账号；客户端更新订阅/取消订阅仍需要
AstrBot `admin` 权限并在群聊中执行，资源操作继续保留管理员权限。

密函查询使用系统中任意可用账号凭据；没有可用凭据时会提示管理员完成登录。角色卡片、角色详情、
角色面板、体力、周报和签到等当前 UID 数据命令仍要求绑定自己的 UID，并继续执行隐私控制。个人
隐私设置本身无需登录，但登录后查询他人仍会遵守隐私规则。
