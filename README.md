...
默认前缀为 `kk`。下列示例均使用默认前缀；如果修改了 `general.command_prefixes`，请把示例中的 `kk` 替换为自己的前缀。完整的 64 条命令、参数和权限见 [`commands.json`](commands.json)，命令使用说明见 [`docs/usage/commands.md`](docs/usage/commands.md)。
