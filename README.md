...
| `cache.refresh_send_card` | `true` | 手动刷新角色面板或基本信息卡片后是否立即发送新卡片；关闭时仍刷新、渲染并更新缓存。 |
...
默认前缀为 `kk`。下列示例均使用默认前缀；如果修改了 `display.command_prefixes`，请把示例中的 `kk` 替换为自己的前缀。完整的 66 条命令、参数和权限见 [`commands.json`](commands.json)，命令使用说明见 [`docs/usage/commands.md`](docs/usage/commands.md)。
