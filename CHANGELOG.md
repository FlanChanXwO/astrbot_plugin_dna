## Goal 4 — T2I、公告并发与订阅管理（2026-09-01）

- 常规卡片、周报、公告和密函链路直接保留 T2I 图片 bytes，避免 PIL 二次编码造成体积膨胀；Pillow 仅用于必要的素材读取/裁切/校验。
- 增加全局短请求并发门、公告 single-flight 与并行详情/图片获取；查询类命令统一支持 At 目标。
- 公告订阅改由运行期 `subscriptions.json` 事实源维护，旧配置群组不再自动恢复；Dashboard 支持真实聊天命令来源目标的启用、停用和删除。
- UID 绑定文案统一改为登录；目标身份固定，不允许 Dashboard 移动 `type/unified_msg_origin/uid`。
...
- 新增版本化客户端更新投递状态：按 `event_key` 固定首次目标，逐目标记录 `pending`/`delivered`，
  下一轮优先重试失败目标，并在取消/停用或全部完成后清理。
...
- 当前迁移阶段已完成查询、订阅、独立轮询、基线状态、定时推送接线和待投递事件状态；
  `client_update_state.json` 的 schema v1 可由当前版本读取并在下一次写入升级，回滚到旧代码前必须恢复
  升级前备份。真实 AstrBot/OneBot 平台发送仍需在受控运行实例中验证。
