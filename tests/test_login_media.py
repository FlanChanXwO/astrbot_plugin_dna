...
def test_login_media_resolver_returns_fixed_urls_only_for_complete_valid_pair(
    tmp_path: Path,
) -> None:
    ...
    assert media.audio_url == (
        "http://localhost:6189/astrbot_plugin_dna/dna/login/media/background.mp3"
    )
    assert str(tmp_path) not in (media.video_url or "")
    assert snapshots.optional_calls == 1


def test_login_media_resolver_disables_pair_when_one_asset_is_missing_or_invalid(
    tmp_path: Path,
) -> None:
    _write_valid_media(tmp_path)
...
def test_login_media_disabled_or_missing_snapshot_does_not_touch_media_files(
    ...
    assert service.file_response("audio", enabled=False).status == 404
...
def _media_routes(service: LoginMediaService):
    return [
        ...
        (LOGIN_MEDIA_AUDIO_ROUTE, lambda: service.file_response("audio"), ["GET"], "音频"),
    ]
...
async def test_login_media_routes_support_range_and_release_generation_lease(
    ...
    assert snapshots.acquire_calls == 2
