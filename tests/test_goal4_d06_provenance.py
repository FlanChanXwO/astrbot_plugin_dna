...
@pytest.mark.asyncio
async def test_announcement_service_marks_chat_command_provenance(tmp_path: Path) -> None:
    ...
    result = await service.subscribe(origin="platform:group:g1", group_id="g1", bot_id="b1")
    assert result.subscription is not None
    assert result.subscription.provenance == "chat_command"


@pytest.mark.asyncio
async def test_legacy_announcement_target_is_not_dashboard_managed(tmp_path: Path) -> None:
    store = SubscriptionStore(tmp_path / "subscriptions.json")
