...
from ..utils.image_utils import convert_img, crop_center_img
...
async def draw_calendar_img(ctx: EventContext):
    ...
    for i, cont in enumerate(content):
        ...
        if isinstance(cont.start_time, int) and cont.start_time > 0:
            dateRange.append(
                datetime.fromtimestamp(cont.start_time, tz=SHANGHAI_TZ).strftime("%Y-%m-%d %H:%M")
            )
        if isinstance(cont.end_time, int) and cont.end_time > 0:
            dateRange.append(
                datetime.fromtimestamp(cont.end_time, tz=SHANGHAI_TZ).strftime("%Y-%m-%d %H:%M")
            )
