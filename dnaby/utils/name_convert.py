...
from ..utils.master_char_const import MASTER_CHAR_ALIAS_TO_NAME, MASTER_CHAR_ID_BY_NAME
...
async def rebuild_name_convert(role_show: RoleShowForTool, is_force: bool = False):
    ...
    writes = (
        (CHAR_ALIAS_PATH, char_alias),
        (WEAPON_ALIAS_PATH, weapon_alias),
        (ID2NAME_PATH, id2name),
    )
...
def alias_to_char_name(char_name: str | None) -> str | None:
    ...
    for i, aliases in char_alias_data.items():
        if (char_name in i) or (char_name in aliases):
            return i
    return None
...
def alias_to_weapon_name(weapon_name: str) -> str:
    for i, aliases in weapon_alias_data.items():
        if (weapon_name in i) or (weapon_name in aliases):
            return i
