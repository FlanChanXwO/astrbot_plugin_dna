...
from ..utils.image import compress_to_webp, save_webp_img
from ..utils.image_utils import change_ev_image_to_bytes
...
from ..utils.segments import MessageSegment, Segment
from ..utils.session import EventContext, Sender

_IMAGE_SUFFIXES = frozenset(Image.registered_extensions())


async def upload_role_panel_img(sender: Sender, ctx: EventContext, char_name: str) -> None:
    if not ctx.image_list:
        _ = await send_dna_notify(sender, ctx, "请随命令发送一张面板图")
        return

    real_char_name = alias_to_char_name(char_name)
    if real_char_name is None:
        _ = await dna_not_found(sender, ctx, f"角色别名【{char_name}】")
        return

    char_id = char_name_to_char_id(real_char_name)
    if char_id is None:
        _ = await dna_not_found(sender, ctx, f"角色【{char_name}】的CharId")
        return

    saved_count = 0
    failed_count = 0
    for image_source in ctx.image_list:
        try:
    ...
    if saved_count == 0:
        _ = await send_dna_notify(sender, ctx, "面板图保存失败")
        return

    msg = f"已上传{real_char_name}面板图{saved_count}张"
    if failed_count:
        msg += f"，失败{failed_count}张"
    _ = await send_dna_notify(sender, ctx, msg)


async def delete_original_role_panel_img(sender: Sender, ctx: EventContext) -> None:
    if ctx.reply is None:
        _ = await send_dna_notify(sender, ctx, "请引用角色面板图")
        return

    image_path = await asyncio.to_thread(delete_original_image, ctx.reply)
    if image_path is None:
        _ = await send_dna_notify(sender, ctx, "未找到对应原图")
        return

    _ = await send_dna_notify(sender, ctx, f"已删除原图：{image_path.name}")


async def delete_role_panel_img_by_id(sender: Sender, ctx: EventContext, char_name: str, image_id: str) -> None:
    real_char_name = alias_to_char_name(char_name)
    if real_char_name is None:
        _ = await dna_not_found(sender, ctx, f"角色别名【{char_name}】")
        return
    ...
    if image_path is None:
        _ = await send_dna_notify(sender, ctx, f"未找到{real_char_name}面板图：{image_id}")
        return

    _ = await send_dna_notify(sender, ctx, f"已删除{real_char_name}面板图：{image_path.stem}")


async def delete_all_role_panel_imgs(sender: Sender, ctx: EventContext, char_name: str) -> None:
    real_char_name = alias_to_char_name(char_name)
    if real_char_name is None:
        _ = await dna_not_found(sender, ctx, f"角色别名【{char_name}】")
        return
    ...
    if deleted_count is None:
        _ = await send_dna_notify(sender, ctx, f"暂无{real_char_name}面板图")
        return

    _ = await send_dna_notify(sender, ctx, f"已删除{real_char_name}全部面板图：{deleted_count}张")


async def list_role_panel_imgs(sender: Sender, ctx: EventContext, char_name: str) -> None:
    real_char_name = alias_to_char_name(char_name)
    if real_char_name is None:
        _ = await dna_not_found(sender, ctx, f"角色别名【{char_name}】")
        return
    ...
    messages: list[Segment] = [MessageSegment.text(f"{real_char_name}面板图列表：共{len(panel_paths)}张\n")]
    for image_path in panel_paths:
        messages.append(MessageSegment.text(f"\nID：{image_path.stem}\n"))
        messages.append(MessageSegment.image(str(image_path)))
    await sender.send(messages)


async def compress_role_panel_imgs(sender: Sender, ctx: EventContext) -> None:
    panel_paths = await asyncio.to_thread(get_all_role_panel_paths)
    if not panel_paths:
        _ = await send_dna_notify(sender, ctx, "暂无角色面板图")
        return
