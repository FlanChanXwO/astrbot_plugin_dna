...
from ..dna_config import get_core_config
...
if TYPE_CHECKING:
    from .local_server import LocalLoginServer

# AstrBot Web API 挂载前缀（main.py 用 context.register_web_api 注册）
...
async def get_dna_login_url() -> str:
    ...
    host = get_core_config("host", "127.0.0.1")
    port = get_core_config("port", 6189)
...
async def page_login_local(sender: Sender, ctx: EventContext, url: str) -> None:
    ...
    await send_login(sender, ctx, f"{url}/dna/i/{login_auth}")
    try:
        # 登录会话本身由 TimedCache 维持 600 秒，这里使用同一窗口等待页面提交。
...
def get_routes() -> list[tuple[str, Callable, list[str], str]]:
    """导出 Web 路由，供 main.py 通过 ``context.register_web_api`` 挂载。

    返回 ``(route_path, view_handler, methods, description)``，与 AstrBot
    ``Context.register_web_api`` 的参数顺序一致。
    """
    return [
