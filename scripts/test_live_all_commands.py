import asyncio
import gc
import json
import os
import re
import sys
import traceback
from datetime import datetime, timezone
from pathlib import Path

from PIL import Image

sys.path.insert(0, str(Path(__file__).resolve().parents[1]))

from astrbot.api.star import StarTools

from src.bootstrap import build_runtime
from src.entry.commands import (
    CommandRegistry,
    CommandRequest,
    execute_use_case,
    load_command_registry,
)
from src.entry.event import EventActor
from src.entry.response import ChainResponse, ImageResponse, PlainTextResponse
from src.infrastructure.persistence.database import AsyncDatabase


class MockContext:
    def __init__(self) -> None:
        self.sent_messages: list[tuple[tuple[object, ...], dict[str, object]]] = []

    async def send_message(self, *args: object, **kwargs: object) -> None:
        self.sent_messages.append((args, kwargs))


def _save_reports(
    output_dir: Path, report_data: dict[str, object], markdown_text: str
) -> tuple[Path, Path]:
    output_dir.mkdir(parents=True, exist_ok=True)
    report_json_path = output_dir / "live_test_report.json"
    report_md_path = output_dir / "live_test_report.md"

    with open(report_json_path, "w", encoding="utf-8") as f:
        json.dump(report_data, f, ensure_ascii=False, indent=2)

    with open(report_md_path, "w", encoding="utf-8") as f:
        f.write(markdown_text)

    return report_json_path, report_md_path


async def _execute_single_command(
    test_id: str,
    msg_text: str,
    desc: str,
    registry: CommandRegistry,
    runtime_services: dict[str, object],
    actor: EventActor,
) -> dict[str, object]:
    print(f"\n>>> Testing: {test_id} ({msg_text}) [{desc}]")
    matched_spec = None
    matched_params = {}
    for spec in registry:
        m = re.match(spec.pattern, msg_text)
        if m is not None:
            matched_spec = spec
            matched_params = m.groupdict()
            break

    if not matched_spec:
        print(f"❌ No matching command spec for: {msg_text}")
        return {
            "test_id": test_id,
            "command": msg_text,
            "description": desc,
            "status": "NO_SPEC_MATCH",
            "responses": [],
        }

    req = CommandRequest(
        command_id=matched_spec.id,
        text=msg_text,
        parameters=matched_params,
        actor=actor,
        target_user_id=None,
        reply_id=None,
        services=runtime_services,
        images=(),
    )

    try:
        responses = []
        async for resp in execute_use_case(matched_spec, req, registry):
            responses.append(resp)

        print(f"   Count of responses: {len(responses)}")
        resp_summary: list[dict[str, object]] = []
        for i, r in enumerate(responses):
            if isinstance(r, PlainTextResponse):
                text_sample = r.text.replace("\n", " ")
                print(f"   [{i}] PlainText(need_at={r.need_at}): {text_sample[:150]}")
                resp_summary.append(
                    {
                        "type": "PlainText",
                        "need_at": r.need_at,
                        "text": r.text,
                    }
                )
            elif isinstance(r, ImageResponse):
                img_path = r.image
                img_info: dict[str, object] = {"type": "Image"}
                if isinstance(img_path, (str, Path)) and os.path.exists(str(img_path)):
                    size_bytes = os.path.getsize(str(img_path))
                    try:
                        with Image.open(str(img_path)) as im:
                            w, h = im.size
                            fmt = im.format
                            print(
                                f"   [{i}] Image: path={img_path}, size={size_bytes:,}B, res={w}x{h}, fmt={fmt}"
                            )
                            img_info.update(
                                {
                                    "path": str(img_path),
                                    "size_bytes": size_bytes,
                                    "resolution": f"{w}x{h}",
                                    "format": fmt,
                                }
                            )
                    except (OSError, ValueError) as e:
                        print(
                            f"   [{i}] Image: path={img_path}, size={size_bytes:,}B, error: {e}"
                        )
                        img_info.update(
                            {
                                "path": str(img_path),
                                "size_bytes": size_bytes,
                                "error": str(e),
                            }
                        )
                elif isinstance(img_path, bytes):
                    print(f"   [{i}] Image: raw_bytes, size={len(img_path):,}B")
                    img_info.update({"size_bytes": len(img_path), "raw": True})
                else:
                    print(f"   [{i}] Image: path={img_path} (NOT FOUND ON DISK)")
                    img_info.update({"path": str(img_path), "missing": True})
                resp_summary.append(img_info)
            elif isinstance(r, ChainResponse):
                print(f"   [{i}] ChainResponse: {r.components}")
                resp_summary.append(
                    {
                        "type": "ChainResponse",
                        "components": str(r.components),
                    }
                )
            else:
                print(f"   [{i}] Other response: {type(r).__name__} -> {r}")
                resp_summary.append(
                    {
                        "type": type(r).__name__,
                        "value": str(r),
                    }
                )
        gc.collect()
        return {
            "test_id": test_id,
            "command": msg_text,
            "description": desc,
            "status": "OK",
            "responses": resp_summary,
        }
    except Exception as exc:  # noqa: BLE001
        print(f"❌ Error executing {test_id}: {exc}")
        traceback.print_exc()
        gc.collect()
        return {
            "test_id": test_id,
            "command": msg_text,
            "description": desc,
            "status": "ERROR",
            "error": str(exc),
            "responses": [],
        }


async def run() -> None:
    print("========================================")
    print("Initializing Plugin Runtime with live DB")
    print("========================================")

    prod_data_dir = Path("/AstrBot/data/plugin_data/astrbot_plugin_dna")
    if (prod_data_dir / "dnaby.sqlite3").exists():
        data_dir = prod_data_dir
    else:
        data_dir = Path(StarTools.get_data_dir("astrbot_plugin_dna"))

    db = AsyncDatabase.from_data_dir(data_dir)
    await db.create_schema_for_tests()
    print(f"Database: {db.path}")

    registry = load_command_registry()
    ctx = MockContext()
    runtime = build_runtime(ctx, None, command_registry=registry, database=db)
    services = runtime.services

    # Test user: 308597424 (QQ) with active UID 1002631141868 (江上月)
    user_id = "308597424"
    bot_id = "onebot"
    group_id = "1007417650"

    actor = EventActor(
        user_id=user_id,
        bot_id=bot_id,
        group_id=group_id,
        unified_msg_origin=f"onebot:{group_id}:{user_id}",
    )

    test_commands = [
        # 0. 帮助
        ("dna帮助", "dna帮助", "帮助菜单卡片渲染"),
        # 1. 账号与隐私控制
        ("dna登录", "dna登录", "生成登录二维码/URL会话"),
        ("dna查看UID", "dna查看UID", "查看绑定UID"),
        ("dna获取ck", "dna获取ck", "查看凭据状态"),
        ("dna绑定1002631141868", "dna绑定1002631141868", "重新/重复绑定测试UID"),
        ("dna开偷窥", "dna开偷窥", "开启个人偷窥"),
        ("dna防偷窥", "dna防偷窥", "关闭个人偷窥"),
        ("dna隐藏UID", "dna隐藏UID", "开启隐藏UID"),
        ("dna显示UID", "dna显示UID", "关闭隐藏UID"),
        ("dna切换1002631141868", "dna切换1002631141868", "切换当前激活UID"),
        ("dna指定开偷窥", "dna指定开偷窥", "管理员指定开偷窥提示"),
        ("dna指定防偷窥", "dna指定防偷窥", "管理员指定防偷窥提示"),
        ("dna指定隐藏UID", "dna指定隐藏UID", "管理员指定隐藏UID提示"),
        ("dna指定显示UID", "dna指定显示UID", "管理员指定显示UID提示"),
        ("dna全体隐藏UID", "dna全体隐藏UID", "管理员全体隐藏UID"),
        ("dna全体显示UID", "dna全体显示UID", "管理员全体显示UID"),
        ("dna取消全体UID隐藏", "dna取消全体UID隐藏", "管理员取消全体UID隐藏"),
        ("dna全体开偷窥", "dna全体开偷窥", "管理员全体开偷窥"),
        ("dna全体防偷窥", "dna全体防偷窥", "管理员全体防偷窥"),
        ("dna取消全体偷窥", "dna取消全体偷窥", "管理员取消全体偷窥"),
        # 2. 玩家卡片与详情
        ("dna卡片", "dna卡片", "玩家总览卡片T2I渲染"),
        ("dna菲娜详情", "dna菲娜详情", "角色属性与武器详情卡T2I渲染"),
        ("dna菲娜详情+暗月+暗月", "dna菲娜详情+暗月+暗月", "角色与自定义武器面板详情"),
        ("dna原图", "dna原图", "角色原图查询（提示暂不支持）"),
        ("dna原图删除", "dna原图删除", "删除角色原图"),
        # 3. 百科、周报与便签模块
        ("dna便签", "dna便签", "实时便签与体力卡片T2I渲染"),
        ("dna周报", "dna周报", "本周周报数据统计T2I渲染"),
        ("dna本周周报", "dna本周周报", "本周周报别名触发"),
        ("dna上周周报", "dna上周周报", "上周周报历史数据统计T2I渲染"),
        ("dna日历", "dna日历", "活动与卡池日历卡片T2I渲染"),
        ("dna兑换码", "dna兑换码", "可用兑换码查询"),
        ("dna菲娜别名", "dna菲娜别名", "角色别名查询"),
        ("dna角色列表", "dna角色列表", "全角色清单查询"),
        ("dna武器列表", "dna武器列表", "全武器清单查询"),
        ("dna菲娜图鉴", "dna菲娜图鉴", "角色图鉴查询"),
        ("dna菲娜攻略", "dna菲娜攻略", "角色攻略查询"),
        # 4. 签到模块
        ("dna签到", "dna签到", "手动执行签到任务"),
        ("dna签到日历", "dna签到日历", "当月签到记录日历T2I渲染"),
        ("dna订阅签到结果", "dna订阅签到结果", "订阅签到广播"),
        ("dna取消订阅签到结果", "dna取消订阅签到结果", "取消订阅签到广播"),
        # 5. 密函模块
        ("dna密函", "dna密函", "密函卡片T2I渲染"),
        ("dna密函列表", "dna密函列表", "可订阅密函类型列表"),
        ("dna订阅扼守密函", "dna订阅扼守密函", "订阅指定密函并@用户"),
        ("dna我的密函", "dna我的密函", "查看当前订阅密函状态"),
        ("dna订阅密函时间17:23", "dna订阅密函时间17:23", "设置密函推送时间范围"),
        ("dna订阅密函图片", "dna订阅密函图片", "开启密函图片推送"),
        ("dna取消订阅密函图片", "dna取消订阅密函图片", "关闭密函图片推送"),
        ("dna订阅密函文本", "dna订阅密函文本", "开启密函文本推送"),
        ("dna取消订阅密函文本", "dna取消订阅密函文本", "关闭密函文本推送"),
        ("dna密函测试", "dna密函测试", "密函推送测试"),
        ("dna取消订阅扼守密函", "dna取消订阅扼守密函", "取消订阅指定密函"),
        # 6. 公告模块
        ("dna公告", "dna公告", "最新公告列表T2I渲染"),
        ("dna公告 1", "dna公告 1", "指定序号公告详情查询"),
        ("dna订阅公告", "dna订阅公告", "群聊订阅游戏公告"),
        ("dna取消订阅公告", "dna取消订阅公告", "群聊退订游戏公告"),
        # 7. 运维与资源模块
        ("dna资源状态", "dna资源状态", "公共资源目录健康检查"),
        ("dna菲娜面板图列表", "dna菲娜面板图列表", "查询自定义面板图"),
        ("dna上传菲娜面板图", "dna上传菲娜面板图", "上传面板图缺失图片提示"),
        ("dna删除菲娜面板图test", "dna删除菲娜面板图test", "删除指定面板图"),
        ("dna删除菲娜全部面板图", "dna删除菲娜全部面板图", "删除全部面板图"),
        ("dna压缩面板图", "dna压缩面板图", "压缩自定义面板图"),
    ]

    output_dir = Path(__file__).resolve().parent / "output"
    results: list[dict[str, object]] = []

    for test_id, msg_text, desc in test_commands:
        res = await _execute_single_command(
            test_id=test_id,
            msg_text=msg_text,
            desc=desc,
            registry=registry,
            runtime_services=services,
            actor=actor,
        )
        results.append(res)

    # Configurable Prefix Tests
    prefix_tests = [
        ("dna", "dna卡片", "dna卡片 (前缀配置为dna)", "自定义前缀dna卡片指令渲染"),
        ("dna", "dna帮助", "dna帮助 (前缀配置为dna)", "自定义前缀dna帮助指令渲染"),
        ("", "卡片", "卡片 (无前缀配置)", "无前缀卡片指令渲染"),
    ]

    print("\n========================================")
    print("Testing Configurable Prefix Variations")
    print("========================================")

    for pfx, cmd_text, test_title, test_desc in prefix_tests:
        pfx_registry = load_command_registry(prefix=pfx)
        pfx_runtime = build_runtime(
            ctx,
            {"display": {"command_prefix": pfx}},
            command_registry=pfx_registry,
            database=db,
        )
        res = await _execute_single_command(
            test_id=test_title,
            msg_text=cmd_text,
            desc=test_desc,
            registry=pfx_registry,
            runtime_services=pfx_runtime.services,
            actor=actor,
        )
        results.append(res)

    # Output report files
    timestamp = datetime.now(timezone.utc).astimezone().strftime("%Y-%m-%d %H:%M:%S")

    md_lines = [
        "# 二重螺旋插件 (dnaby) 全量指令测试报告\n\n",
        f"- **测试时间**: {timestamp}\n",
        f"- **测试账号**: QQ `{user_id}` (UID: `1002631141868` / 角色: 江上月)\n",
        f"- **测试命令总数**: {len(results)}\n\n",
        "| 序号 | 测试指令 | 功能描述 | 执行状态 | 输出摘要 |\n",
        "| :--- | :--- | :--- | :--- | :--- |\n",
    ]

    for i, item in enumerate(results, 1):
        status = item["status"]
        status_badge = "✅ OK" if status == "OK" else f"❌ {status}"
        responses = item.get("responses", [])
        summary_parts = []
        if isinstance(responses, list):
            for r in responses:
                if isinstance(r, dict):
                    t = r.get("type")
                    if t == "PlainText":
                        summary_parts.append(
                            f"文本: `{str(r.get('text', '')).replace(chr(10), ' ')[:60]}`"
                        )
                    elif t == "Image":
                        res = r.get("resolution", "N/A")
                        sz = r.get("size_bytes", 0)
                        summary_parts.append(f"图片: `{res}` ({sz:,}B)")
                    else:
                        summary_parts.append(f"{t}")
        summary_str = (
            "<br>".join(summary_parts)
            if summary_parts
            else (str(item.get("error", "无返回"))[:60])
        )
        md_lines.append(
            f"| {i} | `{item['command']}` | {item['description']} | {status_badge} | {summary_str} |\n"
        )

    report_json_path, report_md_path = _save_reports(
        output_dir,
        {
            "generated_at": timestamp,
            "user_id": user_id,
            "total_tested": len(results),
            "results": results,
        },
        "".join(md_lines),
    )

    print("\n========================================")
    print("SUMMARY OF ALL TESTED COMMANDS")
    print("========================================")
    for item in results:
        print(f"{item['command']!s:<35} : {item['status']}")
    print(f"\nSaved test reports to:\n - {report_json_path}\n - {report_md_path}")


if __name__ == "__main__":
    asyncio.run(run())
